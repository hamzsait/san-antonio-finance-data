"""Build Opus research batch inputs for the Councilwoman Sukh Kaur (D1) donor pool.

Pool = donors to Sukh Kaur's District 1 campaigns (2023 + 2025) who gave >= $100
...
to the Kaur campaign). SA advantage over the Austin preps: occupations and
employer_strings are filer-reported schedule data, not blanks — researchers
start with real identity anchors.

Outputs: kaur_research/kaurbatch_NN.json (20 donors each)
"""
import glob
import json
import os
...
# (all *_research dirs — Jones/Kaur cross-over donors must not be re-scrubbed)
covered_ids = set()
...
print(f"donors >= ${MIN_TOTAL} to the Kaur campaign: {len(donors)}")
...
for i in range(0, len(pool), BATCH):
    ...
    out = os.path.join(ROOT, f"kaurbatch_{i // BATCH + 1:02d}.json")
