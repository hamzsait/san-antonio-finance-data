/* Sukh Kaur (D1) donor research driver: runs headless `claude -p --model
 * claude-opus-4-8` donor-research jobs over all pending kaurbatch_*.json
 * files (the >=$100 Kaur D1 donor pool) with a worker pool,
 ...
 *
 * Logs per-batch token usage + cost to _kaur_usage_log.jsonl.
 * Globs only kaurbatch_* in this directory.
 */
...
const USAGE_LOG = path.join(ROOT, '_kaur_usage_log.jsonl');
...
function pendingBatches() {
  return fs.readdirSync(ROOT)
    .filter(f => /^kaurbatch_\d+\.json$/.test(f))
...
Task: holistic web research on the DONORS in ${path.join(ROOT, batchFile)} (donor batch format; note the dollar field is "site_total" — here it means total given to Sukh Kaur's San Antonio City Council District 1 campaigns: first elected in the June 2023 runoff over incumbent Mario Bravo, re-elected in the June 2025 runoff over Patty Gibbons. She is the sitting D1 councilwoman — the district covering downtown San Antonio and the near-north side). These are donors to a sitting big-city councilmember. Research each; corroborate identity with zip/location/donation pattern before classifying. For every donor, regardless of amount, run the FEC PAC-contribution search, the Texas lobbyist registry search, and a full bio-page read, then record any affiliations found across the full balanced category set. Use the categorize-by-employer fallback (no forced affiliation guess) when no public profile is discoverable beyond a generic occupation.
  ...
  return new Promise(resolve => {
    ...
    child.on('close', code => {
      ...
      if (!fs.existsSync(outPath) || goodOutput(batchFile)) {
        fs.appendFileSync(path.join(ROOT, '_kaur_job_errors.log'),
