"""Apply Ivalis Meza Gonzalez (D8) donor research results to the DB.

Ported from austin-finance-data/d3_research/_apply_d3_results.py -- same
columns, same confidence gate, same dedup rule -- globs
gonzalezbatch_*_results.json in this directory. civic_affiliations already
exists from the earlier SA scrubs; the profile template renders the
affiliation cards from it automatically.

  gonzalezbatch_*_results.json -> donor_identities.resolved_industry /
...
Usage:
    python _apply_gonzalez_results.py            # apply
    python _apply_gonzalez_results.py --dry-run  # report only, no writes
"""
import glob
import json
import os
...
cat_counts = {}
files = sorted(glob.glob(os.path.join(ROOT, "gonzalezbatch_*_results.json")))
