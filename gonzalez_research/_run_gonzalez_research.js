/* Ivalis Meza Gonzalez (D8) donor research driver: runs headless `claude -p --model
 * claude-opus-4-8` donor-research jobs over all pending gonzalezbatch_*.json
 * files (the >=$100 Gonzalez D8 donor pool) with a worker pool,
 ...
 *
 * Logs per-batch token usage + cost to _gonzalez_usage_log.jsonl.
 * Globs only gonzalezbatch_* in this directory.
 */
...
const USAGE_LOG = path.join(ROOT, '_gonzalez_usage_log.jsonl');
...
function pendingBatches() {
  return fs.readdirSync(ROOT)
    .filter(f => /^gonzalezbatch_\d+\.json$/.test(f))
...
Task: holistic web research on the DONORS in ${path.join(ROOT, batchFile)} (donor batch format; note the dollar field is "site_total" — here it means total given to Ivalis Meza Gonzalez's 2025 San Antonio City Council District 8 campaign — her first and only city campaign, so every dollar in the pool is 2024-2025 money: a City Hall staffer turned officeholder — intergovernmental and community relations at the San Antonio River Authority, then Mayor Ron Nirenberg's office from 2018 as director of policy and public engagement, rising to chief of staff, the post she left to run. She led a six-way May 3, 2025 general with 40% and won the June 7, 2025 runoff over Paula McGee 57.4-42.6, succeeding term-limited Manny Pelaez. She is the sitting D8 councilwoman — the district covering San Antonio's Northwest side. Note her surname is the compound "Meza Gonzalez"; she files as "Ivalis Gonzalez" and is also styled "Ivalis Gonzalez Meza" in older coverage. Gonzalez is a very common surname in Bexar County, so corroborate every donor identity on zip and occupation before classifying, and do not assume a shared surname implies a family or business tie). These are donors to a sitting big-city councilmember. Research each; corroborate identity with zip/location/donation pattern before classifying. For every donor, regardless of amount, run the FEC PAC-contribution search, the Texas lobbyist registry search, and a full bio-page read, then record any affiliations found across the full balanced category set. Use the categorize-by-employer fallback (no forced affiliation guess) when no public profile is discoverable beyond a generic occupation.
  ...
  return new Promise(resolve => {
    ...
    child.on('close', code => {
      ...
      if (!fs.existsSync(outPath) || goodOutput(batchFile)) {
        fs.appendFileSync(path.join(ROOT, '_gonzalez_job_errors.log'),
