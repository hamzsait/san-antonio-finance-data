"""Build Opus research batch inputs for the Councilwoman Ivalis Meza Gonzalez (D8) donor pool.

Pool = donors to Ivalis Meza Gonzalez's District 8 campaign (2025) who gave
>= $100 **to this campaign** and aren't already covered
...
to the Gonzalez campaign). SA advantage over the Austin preps: occupations and
employer_strings are filer-reported schedule data, not blanks — researchers
start with real identity anchors.

Outputs: gonzalez_research/gonzalezbatch_NN.json (20 donors each)
"""
import glob
import json
import os
...
print(f"donors >= ${MIN_TOTAL} to the Gonzalez campaign: {len(donors)}")
...
for i in range(0, len(pool), BATCH):
    ...
    out = os.path.join(ROOT, f"gonzalezbatch_{i // BATCH + 1:02d}.json")
