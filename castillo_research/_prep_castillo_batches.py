"""Build Opus research batch inputs for the Councilwoman Teri Castillo (D5) donor pool.

Pool = donors to Teri Castillo's District 5 campaigns (2021, 2023, 2025)
...
to the Castillo campaign). SA advantage over the Austin preps: occupations and
employer_strings are filer-reported schedule data, not blanks — researchers
start with real identity anchors.

Outputs: castillo_research/castillobatch_NN.json (20 donors each)
"""
import glob
import json
import os
...
print(f"donors >= ${MIN_TOTAL} to the Castillo campaign: {len(donors)}")
...
for i in range(0, len(pool), BATCH):
    ...
    out = os.path.join(ROOT, f"castillobatch_{i // BATCH + 1:02d}.json")
