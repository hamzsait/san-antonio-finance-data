/* Teri Castillo (D5) donor research driver: runs headless `claude -p --model
 * claude-opus-4-8` donor-research jobs over all pending castillobatch_*.json
 * files (the >=$100 Castillo D5 donor pool) with a worker pool,
 ...
 *
 * Logs per-batch token usage + cost to _castillo_usage_log.jsonl.
 * Globs only castillobatch_* in this directory.
 */
...
const USAGE_LOG = path.join(ROOT, '_castillo_usage_log.jsonl');
...
function pendingBatches() {
  return fs.readdirSync(ROOT)
    .filter(f => /^castillobatch_\d+\.json$/.test(f))
...
Task: holistic web research on the DONORS in ${path.join(ROOT, batchFile)} (donor batch format; note the dollar field is "site_total" — here it means total given to Teri Castillo's San Antonio City Council District 5 campaigns: a housing organizer and former educator who led a 10-candidate field and won the June 2021 runoff over Rudy Lopez, then was re-elected outright in May 2023 and May 2025. She is the sitting D5 councilwoman — the district covering San Antonio's near West Side, including the Historic Westside where she organized before running). These are donors to a sitting big-city councilmember. Research each; corroborate identity with zip/location/donation pattern before classifying. For every donor, regardless of amount, run the FEC PAC-contribution search, the Texas lobbyist registry search, and a full bio-page read, then record any affiliations found across the full balanced category set. Use the categorize-by-employer fallback (no forced affiliation guess) when no public profile is discoverable beyond a generic occupation.
  ...
  return new Promise(resolve => {
    ...
    child.on('close', code => {
      ...
      if (!fs.existsSync(outPath) || goodOutput(batchFile)) {
        fs.appendFileSync(path.join(ROOT, '_castillo_job_errors.log'),
