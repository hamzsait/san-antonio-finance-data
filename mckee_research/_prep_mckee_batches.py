"""Build Opus research batch inputs for the Councilman Jalen McKee-Rodriguez (D2) donor pool.

Pool = donors to Jalen McKee-Rodriguez's District 2 campaigns (2021, 2023, 2025) who gave >= $100
lifetime and aren't already covered by a prior research batch or an existing
civic_affiliations row. Ported from austin-finance-data/d3_research (the
proven district-scrub prep). Prior coverage = every batch in every sibling
*_research directory (Jones cross-over donors must not be re-scrubbed).
...
to the McKee-Rodriguez campaign). SA advantage over the Austin preps: occupations and
employer_strings are filer-reported schedule data, not blanks — researchers
start with real identity anchors.

Outputs: mckee_research/mckeebatch_NN.json (20 donors each)
"""
import glob
import json
import os
...
# (all *_research dirs — Jones/McKee-Rodriguez cross-over donors must not be re-scrubbed)
covered_ids = set()
...
print(f"donors >= ${MIN_TOTAL} to the McKee-Rodriguez campaign: {len(donors)}")
...
for i in range(0, len(pool), BATCH):
    ...
    out = os.path.join(ROOT, f"mckeebatch_{i // BATCH + 1:02d}.json")
