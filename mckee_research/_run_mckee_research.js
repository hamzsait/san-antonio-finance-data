/* Jalen McKee-Rodriguez (D2) donor research driver: runs headless `claude -p --model
 * claude-opus-4-8` donor-research jobs over all pending mckeebatch_*.json
 * files (the >=$100 McKee-Rodriguez D2 donor pool) with a worker pool,
 ...
 *
 * Logs per-batch token usage + cost to _mckee_usage_log.jsonl.
 * Globs only mckeebatch_* in this directory.
 */
...
const USAGE_LOG = path.join(ROOT, '_mckee_usage_log.jsonl');
...
function pendingBatches() {
  return fs.readdirSync(ROOT)
    .filter(f => /^mckeebatch_\d+\.json$/.test(f))
...
Task: holistic web research on the DONORS in ${path.join(ROOT, batchFile)} (donor batch format; note the dollar field is "site_total" — here it means total given to Jalen McKee-Rodriguez's San Antonio City Council District 2 campaigns: first elected in the June 2021 runoff over incumbent Jada Andrews-Sullivan, re-elected outright in May 2023 and May 2025. He is the sitting D2 councilman — the district covering San Antonio's East Side). These are donors to a sitting big-city councilmember. Research each; corroborate identity with zip/location/donation pattern before classifying. For every donor, regardless of amount, run the FEC PAC-contribution search, the Texas lobbyist registry search, and a full bio-page read, then record any affiliations found across the full balanced category set. Use the categorize-by-employer fallback (no forced affiliation guess) when no public profile is discoverable beyond a generic occupation.
  ...
  return new Promise(resolve => {
    ...
    child.on('close', code => {
      ...
      if (!fs.existsSync(outPath) || goodOutput(batchFile)) {
        fs.appendFileSync(path.join(ROOT, '_mckee_job_errors.log'),
