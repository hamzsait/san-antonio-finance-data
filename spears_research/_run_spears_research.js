/* Misty Spears (D9) donor research driver: runs headless `claude -p --model
 * claude-opus-4-8` donor-research jobs over all pending spearsbatch_*.json
 * files (the >=$100 Spears D9 donor pool) with a worker pool,
 ...
 *
 * Logs per-batch token usage + cost to _spears_usage_log.jsonl.
 * Globs only spearsbatch_* in this directory.
 */
...
const USAGE_LOG = path.join(ROOT, '_spears_usage_log.jsonl');
...
function pendingBatches() {
  return fs.readdirSync(ROOT)
    .filter(f => /^spearsbatch_\d+\.json$/.test(f))
...
Task: holistic web research on the DONORS in ${path.join(ROOT, batchFile)} (donor batch format; note the dollar field is "site_total" — here it means total given to Misty Spears's 2025 San Antonio City Council District 9 campaign — her first and only city campaign, so every dollar in the pool is 2024-2026 money: an accountant-turned-paralegal-turned-county-staffer — B.B.A. accounting from Texas Tech, accountant at Clear Channel Communications and Pioneer Drilling, senior paralegal in municipal law, then Director of Constituent Services for Bexar County Commissioner Grant Moody, the post she left to run. She led a seven-way May 3, 2025 general with 38.0% and won the June 7, 2025 runoff over Angi Taylor Aramburu 56.7-43.3, succeeding term-limited John Courage. She is the sitting D9 councilwoman — the district covering San Antonio's Far North Side — and was GOP-backed in the nonpartisan race (she ran for Bexar County District Clerk as a Republican in 2022), so expect a more conservative-leaning donor pool than prior members. Corroborate every donor identity on zip and occupation before classifying, and do not assume a shared surname implies a family or business tie). These are donors to a sitting big-city councilmember. Research each; corroborate identity with zip/location/donation pattern before classifying. For every donor, regardless of amount, run the FEC PAC-contribution search, the Texas lobbyist registry search, and a full bio-page read, then record any affiliations found across the full balanced category set. Use the categorize-by-employer fallback (no forced affiliation guess) when no public profile is discoverable beyond a generic occupation.
  ...
  return new Promise(resolve => {
    ...
    child.on('close', code => {
      ...
      if (!fs.existsSync(outPath) || goodOutput(batchFile)) {
        fs.appendFileSync(path.join(ROOT, '_spears_job_errors.log'),
