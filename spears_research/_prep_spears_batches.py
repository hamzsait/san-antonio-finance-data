"""Build Opus research batch inputs for the Councilwoman Misty Spears (D9) donor pool.

Pool = donors to Misty Spears's District 9 campaign (2025) who gave
...
to the Spears campaign). SA advantage over the Austin preps: occupations and
employer_strings are filer-reported schedule data, not blanks — researchers
start with real identity anchors.

Outputs: spears_research/spearsbatch_NN.json (20 donors each)
"""
import glob
import json
import os
...
print(f"donors >= ${MIN_TOTAL} to the Spears campaign: {len(donors)}")
...
for i in range(0, len(pool), BATCH):
    ...
    out = os.path.join(ROOT, f"spearsbatch_{i // BATCH + 1:02d}.json")
