/* Marina Alderete Gavito (D7) donor research driver: runs headless `claude -p --model
 * claude-opus-4-8` donor-research jobs over all pending gavitobatch_*.json
 * files (the >=$100 Gavito D7 donor pool) with a worker pool,
 ...
 *
 * Logs per-batch token usage + cost to _gavito_usage_log.jsonl.
 * Globs only gavitobatch_* in this directory.
 */
...
const USAGE_LOG = path.join(ROOT, '_gavito_usage_log.jsonl');
...
function pendingBatches() {
  return fs.readdirSync(ROOT)
    .filter(f => /^gavitobatch_\d+\.json$/.test(f))
...
Task: holistic web research on the DONORS in ${path.join(ROOT, batchFile)} (donor batch format; note the dollar field is "site_total" — here it means total given to Marina Alderete Gavito's San Antonio City Council District 7 campaigns: a technology and civic-innovation executive — management roles at Rackspace and USAA, founding executive director of Tech Bloc, then executive director of SA Digital Connects, a public-private partnership on the digital divide — who won the June 10, 2023 runoff over Dan Rossiter and was re-elected outright in May 2025. She is the sitting D7 councilwoman — the district covering San Antonio's Northwest side. Note her surname is the compound "Alderete Gavito"; she files as "Marina Gavito", and an unrelated 2025 opponent was named Cynthia Lugo Alderete, so do not conflate the two when researching donors). These are donors to a sitting big-city councilmember. Research each; corroborate identity with zip/location/donation pattern before classifying. For every donor, regardless of amount, run the FEC PAC-contribution search, the Texas lobbyist registry search, and a full bio-page read, then record any affiliations found across the full balanced category set. Use the categorize-by-employer fallback (no forced affiliation guess) when no public profile is discoverable beyond a generic occupation.
  ...
  return new Promise(resolve => {
    ...
    child.on('close', code => {
      ...
      if (!fs.existsSync(outPath) || goodOutput(batchFile)) {
        fs.appendFileSync(path.join(ROOT, '_gavito_job_errors.log'),
