"""Build Opus research batch inputs for the Councilwoman Marina Alderete Gavito (D7) donor pool.

Pool = donors to Marina Alderete Gavito's District 7 campaigns (2023, 2025)
who gave >= $100 lifetime and aren't already covered
...
to the Gavito campaign). SA advantage over the Austin preps: occupations and
employer_strings are filer-reported schedule data, not blanks — researchers
start with real identity anchors.

Outputs: gavito_research/gavitobatch_NN.json (20 donors each)
"""
import glob
import json
import os
...
print(f"donors >= ${MIN_TOTAL} to the Gavito campaign: {len(donors)}")
...
for i in range(0, len(pool), BATCH):
    ...
    out = os.path.join(ROOT, f"gavitobatch_{i // BATCH + 1:02d}.json")
