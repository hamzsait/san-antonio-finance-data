"""Apply Mayor Jones donor research results to the DB.

Ported from austin-finance-data/d3_research/_apply_d3_results.py -- same
columns, same confidence gate, same dedup rule -- globs
jonesbatch_*_results.json in this directory. Creates civic_affiliations on
first run (this is SA's first scrub; the profile template renders the
affiliation cards from it automatically).

  jonesbatch_*_results.json -> donor_identities.resolved_industry /
...
Usage:
    python _apply_jones_results.py            # apply
    python _apply_jones_results.py --dry-run  # report only, no writes
"""
import glob
import json
import os
...
cat_counts = {}
files = sorted(glob.glob(os.path.join(ROOT, "jonesbatch_*_results.json")))
