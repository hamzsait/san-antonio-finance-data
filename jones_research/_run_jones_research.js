/* Mayor Jones donor research driver: runs headless `claude -p --model
 * claude-opus-4-8` donor-research jobs over all pending jonesbatch_*.json
 * files (the >=$100 Jones 2025 mayoral donor pool) with a worker pool,
 ...
 *
 * Logs per-batch token usage + cost to _jones_usage_log.jsonl.
 * Globs only jonesbatch_* in this directory.
 */
...
const USAGE_LOG = path.join(ROOT, '_jones_usage_log.jsonl');
...
function pendingBatches() {
  return fs.readdirSync(ROOT)
    .filter(f => /^jonesbatch_\d+\.json$/.test(f))
...
Task: holistic web research on the DONORS in ${path.join(ROOT, batchFile)} (donor batch format; note the dollar field is "site_total" — here it means total given to Gina Ortiz Jones's winning 2025 San Antonio mayoral campaign: a 27-candidate May general and a June runoff she won 54.3%. She is the sitting Mayor of San Antonio, formerly Under Secretary of the Air Force and a two-time TX-23 congressional candidate). These are donors to a sitting big-city mayor. Research each; corroborate identity with zip/location/donation pattern before classifying. For every donor, regardless of amount, run the FEC PAC-contribution search, the Texas lobbyist registry search, and a full bio-page read, then record any affiliations found across the full balanced category set. Use the categorize-by-employer fallback (no forced affiliation guess) when no public profile is discoverable beyond a generic occupation.
  ...
  return new Promise(resolve => {
    ...
    child.on('close', code => {
      ...
      if (!fs.existsSync(outPath) || goodOutput(batchFile)) {
        fs.appendFileSync(path.join(ROOT, '_jones_job_errors.log'),
