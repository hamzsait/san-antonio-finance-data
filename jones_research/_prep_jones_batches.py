"""Build Opus research batch inputs for the Mayor Jones donor pool.

Pool = donors to Gina Ortiz Jones's 2025 mayoral campaign who gave >= $100
...
proven district-scrub prep); first SA scrub, so prior coverage is just this
directory's own batches (re-runs must not re-scrub).
...
to the Jones campaign). SA advantage over the Austin preps: occupations and
employer_strings are filer-reported schedule data, not blanks — researchers
start with real identity anchors.

Outputs: jones_research/jonesbatch_NN.json (20 donors each)
"""
import glob
import json
import os
...
covered_ids = set()
for f in sorted(glob.glob(os.path.join(ROOT, "*batch_*.json"))):
...
print(f"donors >= ${MIN_TOTAL} to the Jones campaign: {len(donors)}")
...
for i in range(0, len(pool), BATCH):
    ...
    out = os.path.join(ROOT, f"jonesbatch_{i // BATCH + 1:02d}.json")
