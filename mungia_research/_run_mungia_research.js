/* Edward Mungia (D4) donor research driver: runs headless `claude -p --model
 * claude-opus-4-8` donor-research jobs over all pending mungiabatch_*.json
 * files (the >=$100 Mungia D4 donor pool) with a worker pool,
 ...
 *
 * Logs per-batch token usage + cost to _mungia_usage_log.jsonl.
 * Globs only mungiabatch_* in this directory.
 */
...
const USAGE_LOG = path.join(ROOT, '_mungia_usage_log.jsonl');
...
function pendingBatches() {
  return fs.readdirSync(ROOT)
    .filter(f => /^mungiabatch_\d+\.json$/.test(f))
...
Task: holistic web research on the DONORS in ${path.join(ROOT, batchFile)} (donor batch format; note the dollar field is "site_total" — here it means total given to Edward Mungia's San Antonio City Council District 4 campaign: a first-time candidate who won the open D4 seat outright in May 2025 after nine years as a staffer in the same D4 council office, latterly director of special projects under Adriana Rocha Garcia, whom he succeeded. He is the sitting D4 councilman — the district covering San Antonio's Southwest Side). These are donors to a sitting big-city councilmember. Research each; corroborate identity with zip/location/donation pattern before classifying. For every donor, regardless of amount, run the FEC PAC-contribution search, the Texas lobbyist registry search, and a full bio-page read, then record any affiliations found across the full balanced category set. Use the categorize-by-employer fallback (no forced affiliation guess) when no public profile is discoverable beyond a generic occupation.
  ...
  return new Promise(resolve => {
    ...
    child.on('close', code => {
      ...
      if (!fs.existsSync(outPath) || goodOutput(batchFile)) {
        fs.appendFileSync(path.join(ROOT, '_mungia_job_errors.log'),
