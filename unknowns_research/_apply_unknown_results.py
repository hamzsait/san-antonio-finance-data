"""Apply site-wide UNKNOWNS re-scrub results to the DB.

Ported from whyte_research/_apply_whyte_results.py — same columns, same
confidence gate, same dedup rule — globs unkbatch_*_results.json in this
directory. Adds the v4 categories (arena_venue, arena_opposition,
charter_school, public_school_advocacy) which flow through the same
civic_affiliations rows; rendering buckets live in generate_profile_data.py.

  unkbatch_*_results.json -> donor_identities.resolved_industry /
...
Every donor in this pool has resolved_industry NULL by construction, so the
fill-NULL-only guard never blocks a verdict here. Idempotent: re-applying
overwrites the same fields; affiliations are deduped on
(canonical_name, organization). This must be re-run against the canonical
...
Usage:
    python _apply_unknown_results.py            # apply
    python _apply_unknown_results.py --dry-run  # report only, no writes
"""
import glob
import json
import os
...
cat_counts = {}
files = sorted(glob.glob(os.path.join(ROOT, "unkbatch_*_results.json")))
