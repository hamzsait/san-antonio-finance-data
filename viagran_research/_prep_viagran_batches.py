"""Build Opus research batch inputs for the Councilwoman Phyllis Viagran (D3) donor pool.

Pool = donors to Phyllis Viagran's District 3 campaigns (2021, 2023, 2025) who gave >= $100
...
to the Viagran campaign). SA advantage over the Austin preps: occupations and
employer_strings are filer-reported schedule data, not blanks — researchers
start with real identity anchors.

Outputs: viagran_research/viagranbatch_NN.json (20 donors each)
"""
import glob
import json
import os
...
print(f"donors >= ${MIN_TOTAL} to the Viagran campaign: {len(donors)}")
...
for i in range(0, len(pool), BATCH):
    ...
    out = os.path.join(ROOT, f"viagranbatch_{i // BATCH + 1:02d}.json")
