/* Phyllis Viagran (D3) donor research driver: runs headless `claude -p --model
 * claude-opus-4-8` donor-research jobs over all pending viagranbatch_*.json
 * files (the >=$100 Viagran D3 donor pool) with a worker pool,
 ...
 *
 * Logs per-batch token usage + cost to _viagran_usage_log.jsonl.
 * Globs only viagranbatch_* in this directory.
 */
...
const USAGE_LOG = path.join(ROOT, '_viagran_usage_log.jsonl');
...
function pendingBatches() {
  return fs.readdirSync(ROOT)
    .filter(f => /^viagranbatch_\d+\.json$/.test(f))
...
Task: holistic web research on the DONORS in ${path.join(ROOT, batchFile)} (donor batch format; note the dollar field is "site_total" — here it means total given to Phyllis Viagran's San Antonio City Council District 3 campaigns: first elected in the June 2021 runoff over Tomas Uresti, re-elected outright in May 2023 and May 2025. She is the sitting D3 councilwoman — the district covering San Antonio's South Side. Do not confuse her with her sister Rebecca Viagran, who held the D3 seat 2013-2021). These are donors to a sitting big-city councilmember. Research each; corroborate identity with zip/location/donation pattern before classifying. For every donor, regardless of amount, run the FEC PAC-contribution search, the Texas lobbyist registry search, and a full bio-page read, then record any affiliations found across the full balanced category set. Use the categorize-by-employer fallback (no forced affiliation guess) when no public profile is discoverable beyond a generic occupation.
  ...
  return new Promise(resolve => {
    ...
    child.on('close', code => {
      ...
      if (!fs.existsSync(outPath) || goodOutput(batchFile)) {
        fs.appendFileSync(path.join(ROOT, '_viagran_job_errors.log'),
