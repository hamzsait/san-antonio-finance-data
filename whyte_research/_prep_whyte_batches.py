"""Build Opus research batch inputs for the Councilman Marc Whyte (D10) donor pool.

Pool = donors to Marc Whyte's District 10 campaigns (2023, 2025) who gave
...
to the Whyte campaign). SA advantage over the Austin preps: occupations and
employer_strings are filer-reported schedule data, not blanks — researchers
start with real identity anchors.

Outputs: whyte_research/whytebatch_NN.json (20 donors each)
"""
import glob
import json
import os
...
print(f"donors >= ${MIN_TOTAL} to the Whyte campaign: {len(donors)}")
...
for i in range(0, len(pool), BATCH):
    ...
    out = os.path.join(ROOT, f"whytebatch_{i // BATCH + 1:02d}.json")
